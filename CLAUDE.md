...
| `MphRead -altprobe "ROOM" -at X,Y,Z [-hunter H] [-delay N]` | stand one hunter on one spot, jump, and morph N frames later: does the floor stop working? A table over every delay in the jump's arc, or with `-delay N` one trial printed frame by frame. Written for a report the map sweeps cannot reproduce, because the fault needs two inputs in one order at one place -- `-maptest`'s tour morphs and jumps constantly and has never once been standing where it matters when it does them. It drives a **non-main slot**: slot 0's controls are refilled from the keyboard every frame, which is why the tour's own reports have always read "moved 7/8" |
