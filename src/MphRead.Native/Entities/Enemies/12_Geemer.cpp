...
namespace MphRead::Entities::Enemies
{
    ...
    void Enemy12Entity::EnemyInitialize()
    {
        ...
        {
            facing = RequireReference(_spawner).Transform.Row2().Xyz();
        }
        facing = facing.Normalized(); // the game doesn't do this
        Vector3 up(0.0F, RequireReference(_spawner).Transform.Row1().Y, 0.0F);
        SetTransform(facing, up, RequireReference(_spawner).Position);
        Flags |= EnemyFlags::Visible;
        Flags |= EnemyFlags::OnRadar;
        _health = _healthMax = 12;
        _boundingRadius = 0.25F;
        const EnemySpawnFields00 spawnFields = RequireReference(_spawner).Data.Fields.S00();
        _hurtVolumeInit = CollisionVolume(spawnFields.Volume0);
        if (12 >= static_cast<std::int32_t>(Metadata::EnemyModelNames.size()))
        {
            throw SceneDetail::IndexOutOfRangeException();
        }
        SetUpModel(Metadata::EnemyModelNames[12], 2);
        _extendTimer = 60 * 2; // todo: FPS stuff
        _field19C = _field1A8 = up;
        _angleInc = Fixed::ToFloat(static_cast<std::int32_t>(Rng::GetRandomInt2(0x7000))) + 3.0F;
        _angleInc /= 2.0F; // todo: FPS stuff
        _maxAngle = Fixed::ToFloat(static_cast<std::int32_t>(Rng::GetRandomInt2(0))) + 60.0F;
        _angleCos = std::cos(DegreesToRadians(_angleInc));
        _homeVolume = CollisionVolume::Move(
            spawnFields.Volume1, RequireReference(_spawner).Data.Header.Position.ToFloatVector());
        _direction = Vector3::Cross(facing, up).Normalized();
    }

    void Enemy12Entity::SetAnimation(GeemerAnim anim, AnimFlags flags)
    {
        _models[0].SetAnimation(static_cast<std::int32_t>(anim), flags);
    }

    void Enemy12Entity::EnemyProcess()
    {
        ...
        else if (anim == GeemerAnim::Extend)
        {
            ...
            else if (((*model.AnimInfo->Flags)[0] & AnimFlags::Ended) != AnimFlags::None)
            {
        ...
        if (!_seekingVolume)
        {
            ...
            else if (!_homeVolume.TestPoint(Position))
            {
                ...
                Vector3 facing = WithY(
                    static_cast<Vector3>(Position) - spawner.Data.Header.Position.ToFloatVector(),
                    0.0F);
                if (facing.X == 0.0F && facing.Z == 0.0F)
                {
                    ...
                    if (facing.X == 0.0F && facing.Z == 0.0F)
                    {
                        facing = WithY(spawner.Data.Header.FacingVector.ToFloatVector(), 0.0F);
                    }
                }
        ...
        if (colCount > 0)
        {
            ...
            Vector3 upVec = upVector
                + ScaleVector(_field1A8 - upVector, Fixed::ToFloat(819) / 2.0F); // todo: FPS stuff
    ...
    bool Enemy12Entity::EnemyTakeDamage(EntityBase* source)
    {
        if (source != nullptr && source->Type == EntityType::BeamProjectile)
        {
            BeamProjectileEntity& beam
                = RequireReference(ManagedCast<BeamProjectileEntity>(source));
            const std::int32_t beamIndex = static_cast<std::int32_t>(beam.Beam());
            if (beamIndex < 0
                || static_cast<std::size_t>(beamIndex) >= BeamEffectiveness.size())
            {
                throw SceneDetail::IndexOutOfRangeException();
            }
            if (BeamEffectiveness[static_cast<std::size_t>(beamIndex)] == Effectiveness::Zero)
            {
                const std::int32_t animationIndex = (*_models[0].AnimInfo->Index)[0];
                if (animationIndex == 2)
                {
                    _soundSource.PlaySfx(SfxId::GEEMER_EXTEND);
                    SetAnimation(GeemerAnim::Extend, AnimFlags::NoLoop);
