...
inline constexpr ModuleDescriptor kModule{
    ...
    PortStatus::PartialController, "update_gorea_seal_sphere_2"};

// Enemy32Entity.EnemyInitialize.
void EnemyInitialize(gameplay::EnemyState& agent) noexcept;

// Enemy32Entity.Activate and Deactivate: Gorea opens and closes the
...
// Enemy32Entity.EnemyProcess: the sphere sits on a node of Gorea's
...
// Enemy32Entity.SetDead: the sphere stops being a target for good.
// Distinct from Deactivate, which a later phase can undo.
void SetDead(gameplay::EnemyState& agent) noexcept;

// Enemy32Entity.DamageTimer: how long the chest core stays lit red
...
// Enemy32Entity.EnemyTakeDamage.  The sphere never loses energy: it
