...
namespace
{
    ...
    {
        return Vector3Equal(left.Position, right.Position) && Vector3Equal(left.Normal, right.Normal) && Vector3Equal(left.Color, right.Color) && Vector2Equal(left.Uv, right.Uv) && left.MatrixId == right.MatrixId;
    }
...
namespace MphRead::Export
{
        ...
        const Model &model, bool transformRoom, std::int32_t recolorIndex)
    {
                ...
                std::int32_t textureId, std::int32_t paletteId, const std::string &name)
        {
            ...
            if (textureId != -1 && imagesInLibrary.find(key) == imagesInLibrary.end())
            {
                ...
                if (id <= 0)
                {
                    output += recolor.Name + "/" + std::to_string(textureId) + "-" + std::to_string(paletteId) + ".png";
                }
                else
                {
        ...
        for (const auto &meshRef : RequireReference(model.Meshes))
        {
            ...
            output += "\n\t\t<geometry id=\"geometry" + std::to_string(meshCounter) + "\" name=\"geom" + std::to_string(meshCounter) + "\">";
            output += "\n\t\t\t<mesh>";

            output += "\n\t\t\t\t<source id=\"" + geometryID + "-positions\">";
            const std::string positionsArrayID = geometryID + "-positions-array";
            output += "\n\t\t\t\t\t<float_array id=\"" + positionsArrayID + "\" count=\"" + std::to_string(numVertices * 3) + "\">";
            for (const Vertex &vert : meshVerts)
            {
                output += FloatFormat(vert.Position.X) + " " + FloatFormat(vert.Position.Y) + " " + FloatFormat(vert.Position.Z) + " ";
            }
            output += "</float_array>";

            output += "\n\t\t\t\t\t<technique_common>";
            output += "\n\t\t\t\t\t\t<accessor source=\"#" + positionsArrayID + "\" count=\"" + std::to_string(numVertices) + "\" stride=\"3\">";
            ...
            output += "\n\t\t\t\t\t<float_array id=\"" + normalsArrayID + "\" count=\"" + std::to_string(numVertices * 3) + "\">";
            ...
            output += "\n\t\t\t\t\t<technique_common>";
            output += "\n\t\t\t\t\t\t<accessor source=\"#" + normalsArrayID + "\" count=\"" + std::to_string(numVertices) + "\" stride=\"3\">";
            output += "\n\t\t\t\t\t\t\t<param name=\"X\" type=\"float\" />";
            output += "\n\t\t\t\t\t\t\t<param name=\"Y\" type=\"float\" />";
            output += "\n\t\t\t\t\t\t\t<param name=\"Z\" type=\"float\" />";
            output += "\n\t\t\t\t\t\t</accessor>";
            ...
            output += "\n\t\t\t\t\t<float_array id=\"" + vertexcolorsArrayID + "\" count=\"" + std::to_string(numVertices * 3) + "\">";
            for (const Vertex &vert : meshVerts)
            {
                output += FloatFormat(vert.Color.X) + " " + FloatFormat(vert.Color.Y) + " " + FloatFormat(vert.Color.Z) + " ";
            }
            output += "</float_array>";

            output += "\n\t\t\t\t\t<technique_common>";
            output += "\n\t\t\t\t\t\t<accessor source=\"#" + vertexcolorsArrayID + "\" count=\"" + std::to_string(numVertices) + "\" stride=\"3\">";
            ...
            output += "\n\t\t\t\t\t<float_array id=\"" + texcoordsArrayID + "\" count=\"" + std::to_string(numVertices * 2) + "\">";
            ...
            output += "\n\t\t\t\t\t<technique_common>";
            output += "\n\t\t\t\t\t\t<accessor source=\"#" + texcoordsArrayID + "\" count=\"" + std::to_string(numVertices) + "\" stride=\"2\">";
            ...
            output += "\n\t\t\t\t<triangles material=\"#" + textureName + "-material\" count=\"" + std::to_string(numVertices / 3) + "\">";
            output += "\n\t\t\t\t\t<input semantic=\"VERTEX\" source=\"#" + geometryID + "-vertices\" offset=\"0\" />";
            output += "\n\t\t\t\t\t<input semantic=\"NORMAL\" source=\"#" + geometryID + "-normals\" offset=\"1\" />";
            output += "\n\t\t\t\t\t<input semantic=\"TEXCOORD\" source=\"#" + geometryID + "-texcoords\" offset=\"2\" set=\"1\" />";
            output += "\n\t\t\t\t\t<input semantic=\"COLOR\" source=\"#" + geometryID + "-colors\" offset=\"3\" set=\"0\" />";
            output += "\n\t\t\t\t\t<p>";
        ...
        for (const auto &materialRef : RequireReference(model.Materials))
        {
            ...
            output += "\n\t\t\t\t\t\t</ambient>";
            output += "\n\t\t\t\t\t\t<diffuse>";
            output += "\n\t\t\t\t\t\t\t<texture texture=\"" + textureName + "-sampler\" texcoord=\"UVMap\" />";
            output += "\n\t\t\t\t\t\t</diffuse>";
            output += "\n\t\t\t\t\t\t<specular>";
        ...
        const Model &model, std::int32_t meshId, std::string &output, std::int32_t indent)
    {
        ...
        output += "<instance_material symbol=\"" + textureName + "-material\" target=\"#" + textureName + "-material\">\n";
