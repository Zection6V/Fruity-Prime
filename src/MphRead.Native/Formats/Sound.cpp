...
namespace MphRead::Formats::Sound
{
    namespace
    {
        ...
        [[nodiscard]] std::uint8_t CheckedByte(
            std::span<const std::uint8_t> bytes, std::size_t index)
        {
            if (index >= bytes.size())
            {
                MphRead::ReadDetail::ThrowRange();
        ...
        [[nodiscard]] std::string WaveFormatString(WaveFormat format)
        {
            ...
            case WaveFormat::ADPCM:
                return "ADPCM";
            default:
                return std::to_string(static_cast<std::int32_t>(format));
            }
        }
            ...
            if (format == WaveFormat::ADPCM)
            {
                    ...
                    i = ManagedAdd(i, 1))
                {
                    ...
                    if (stepIndex < 0 || stepIndex >= static_cast<std::int32_t>(
                                                          MphRead::Metadata::AdpcmTable.size()))
                    {
                    ...
                    const std::int32_t step =
                        MphRead::Metadata::AdpcmTable[static_cast<std::size_t>(stepIndex)];
        ...
        [[nodiscard]] std::shared_ptr<std::vector<std::uint8_t>> InterleaveStreamChannels(
            const std::shared_ptr<const std::vector<std::shared_ptr<std::vector<std::uint8_t>>>>&
                channels,
            WaveFormat format)
        {
            if (!channels)
            ...
            auto data =
                std::make_shared<std::vector<std::uint8_t>>(static_cast<std::size_t>(length));
            for (std::int32_t i = 0; i < channelCount; ++i)
            {
                ...
                for (std::int32_t j = 0; j < channelLength; j = ManagedAdd(j, bytesPerSample))
                {
                    ...
                    {
                        if (srcIndex < 0 || srcIndex >= channelLength || destIndex < 0 ||
                            destIndex >= length)
                        {
                            MphRead::ReadDetail::ThrowRange();
                        }
        ...
        [[nodiscard]] WaveFormat ValidateMphFormat(std::uint8_t format)
        {
            if (format > 2)
            {
                throw MphRead::ProgramException(
                    "Invalid wave format " + std::to_string(format) + ".");
            }
        ...
        [[nodiscard]] WaveFormat ValidateFhFormat(std::uint8_t format)
        {
            ...
            throw MphRead::ProgramException(
                "Unexpected FH sound header value: " + std::to_string(format));
        }
    ...
    SoundNativeRuntime::LazyByteArray::LazyByteArray(Factory factory) : _factory(std::move(factory))
    {
    }
    ...
    BinaryWriter::BinaryWriter(std::ostream& stream) noexcept : _stream(&stream)
    {
    }
    ...
    {
        std::array<std::uint8_t, 4> bytes{static_cast<std::uint8_t>(value),
        ...
        if (_pendingHighSurrogate)
        {
            if (value >= 0xDC00 && value <= 0xDFFF)
            {
                const std::uint32_t high =
                    static_cast<std::uint32_t>(*_pendingHighSurrogate - 0xD800);
            ...
            throw std::runtime_error(
                "Unable to translate Unicode character to specified code page.");
        }
    ...
    SdatHeader::SdatHeader(SoundNativeRuntime::Char4 type,
        std::uint32_t magic,
        ...
        SoundNativeRuntime::Bytes16 reserved)
        : Type(std::move(type)), Magic(magic), FileSize(fileSize), HeaderSize(headerSize),
          ...
          FileBlockOffset(fileBlockOffset), FileBlockSize(fileBlockSize),
    ...
    SdatHeader SdatHeader::FromMarshaledBytes(const std::array<std::uint8_t, 64>& bytes)
    {
        ...
        return SdatHeader(ReadCharArray<4>(data, 0),
            ReadLittle<std::uint32_t>(data, 4),
            ReadLittle<std::uint32_t>(data, 8),
            ReadLittle<std::uint16_t>(data, 12),
            ReadLittle<std::uint16_t>(data, 14),
            ReadLittle<std::uint32_t>(data, 16),
            ReadLittle<std::uint32_t>(data, 20),
            ReadLittle<std::uint32_t>(data, 24),
            ReadLittle<std::uint32_t>(data, 28),
            ReadLittle<std::uint32_t>(data, 32),
            ReadLittle<std::uint32_t>(data, 36),
            ReadLittle<std::uint32_t>(data, 40),
            ReadLittle<std::uint32_t>(data, 44),
            ReadByteArray<16>(data, 48));
    }
    BlockHeader::BlockHeader(SoundNativeRuntime::Char4 type,
    ...
    {
        const std::span<const std::uint8_t> data(bytes);
        return BlockHeader(ReadCharArray<4>(data, 0),
            ReadLittle<std::uint32_t>(data, 4),
            ReadLittle<std::uint32_t>(data, 8),
            ReadLittle<std::uint32_t>(data, 12),
            ReadLittle<std::uint32_t>(data, 16),
            ReadLittle<std::uint32_t>(data, 20),
            ReadLittle<std::uint32_t>(data, 24),
            ReadLittle<std::uint32_t>(data, 28),
    ...
    {
        const std::span<const std::uint8_t> data(bytes);
        return SdatFatHeader(ReadCharArray<4>(data, 0),
            ReadLittle<std::uint32_t>(data, 4),
            ReadLittle<std::uint32_t>(data, 8));
    }
    SdatFileHeader::SdatFileHeader(SoundNativeRuntime::Char4 type,
        std::uint32_t size,
        std::uint32_t count,
    ...
    {
        const std::span<const std::uint8_t> data(bytes);
        return SdatFileHeader(ReadCharArray<4>(data, 0),
            ReadLittle<std::uint32_t>(data, 4),
            ReadLittle<std::uint32_t>(data, 8),
            ReadLittle<std::uint32_t>(data, 12));
    }
    ...
    StrmPlayerInfo::StrmPlayerInfo(std::uint8_t channelCount,
        SoundNativeRuntime::Bytes16 channelNumbers,
        SoundNativeRuntime::Bytes7 reserved)
        : ChannelCount(channelCount), ChannelNumbers(std::move(channelNumbers)),
          Reserved(std::move(reserved))
    {
    }
    ...
    SoundStreamHeader::SoundStreamHeader(SoundNativeRuntime::Char4 type,
        std::uint32_t magic,
        std::uint32_t dataSize,
        std::uint16_t size,
        std::uint16_t dataBlocks,
        SoundNativeRuntime::Char4 headType,
        std::uint32_t headerSize,
        std::uint8_t format,
        ...
        : Type(std::move(type)), Magic(magic), DataSize(dataSize), Size(size),
          DataBlocks(dataBlocks), HeadType(std::move(headType)), HeaderSize(headerSize),
          Format(format), LoopFlag(loopFlag), Channels(channels), Padding25(padding25),
          SampleRate(sampleRate), Timer(timer), LoopStart(loopStart), LoopEnd(loopEnd),
          DataOffset(dataOffset), BlockCount(blockCount), BlockSize(blockSize),
          BlockSamples(blockSamples), LastBlockSize(lastBlockSize),
          LastBlockSamples(lastBlockSamples)
    {
    }
    SoundStreamHeader SoundStreamHeader::FromMarshaledBytes(
        const std::array<std::uint8_t, 64>& bytes)
    {
        const std::span<const std::uint8_t> data(bytes);
        return SoundStreamHeader(ReadCharArray<4>(data, 0),
            ReadLittle<std::uint32_t>(data, 4),
            ReadLittle<std::uint32_t>(data, 8),
            ReadLittle<std::uint16_t>(data, 12),
            ReadLittle<std::uint16_t>(data, 14),
            ReadCharArray<4>(data, 16),
            ReadLittle<std::uint32_t>(data, 20),
            data[24],
            data[25],
            data[26],
            data[27],
            ReadLittle<std::uint16_t>(data, 28),
            ReadLittle<std::uint16_t>(data, 30),
            ReadLittle<std::uint32_t>(data, 32),
            ReadLittle<std::uint32_t>(data, 36),
            ReadLittle<std::uint32_t>(data, 40),
            ReadLittle<std::uint32_t>(data, 44),
            ReadLittle<std::uint32_t>(data, 48),
            ReadLittle<std::uint32_t>(data, 52),
            ReadLittle<std::uint32_t>(data, 56),
            ReadLittle<std::uint32_t>(data, 60));
    }
    ...
    SoundStream::SoundStream(std::int32_t id,
        std::string name,
        ...
        : Id(id), Name(std::move(name)), Format(static_cast<WaveFormat>(header.Format)),
          ...
          LoopEnd(header.LoopEnd), _channels(std::move(channels)), Channels(_channels),
          Volume(volume), BufferData(std::make_shared<SoundNativeRuntime::LazyByteArray>(
                              [channelsRef = _channels, format = Format]()
                              { return InterleaveStreamChannels(channelsRef, format); }))
    {
    }
    SoundSample::SoundSample(std::uint32_t id,
        std::uint32_t offset,
        SoundSampleHeader header,
        std::span<const std::uint8_t> data)
        : Id(id), Offset(offset), Format(ValidateMphFormat(header.Format)),
          ...
          WaveData(std::make_shared<SoundNativeRuntime::LazyByteArray>(
              ...
              {
                  return DecodeWaveDataVector(*dataRef,
                      format,
                      GetSampleCountValue(format, sampleStart, sampleLength),
                      false);
              }))
    {
    }
    ...
    SoundSample::SoundSample(std::uint32_t id, NullTag)
        : Id(id), Offset(0), Format(WaveFormat::None), Loop(false), SampleRate(0), SampleStart(0),
          SampleLength(0), LoopStart(0), LoopLength(0),
    ...
    std::span<const std::uint8_t> SoundSample::GetLoop() const
    {
        ...
        return CheckedSlice(
            *data, static_cast<std::uint64_t>(start), static_cast<std::uint64_t>(length));
    }
    ...
    SoundSelectEntry::SoundSelectEntry(std::string name, RawSoundSelectEntry raw)
        : Name(std::move(name)), Id(raw.Id), Type(raw.Type), Field4(raw.Field4), Field8(raw.Field8),
          FieldC(raw.FieldC)
    {
    }
    ...
    MusicTrack::MusicTrack(std::uint32_t id, RawMusicTrack raw)
        : MusicId(static_cast<MphRead::MusicId>(id)),
          SeqId(raw.SeqId == std::numeric_limits<std::uint16_t>::max()
                    ? MphRead::SeqId::None
                    : static_cast<MphRead::SeqId>(raw.SeqId)),
    ...
    SfxScriptFile::SfxScriptFile(std::string name,
        ...
        : Name(std::move(name)), Header(header),
          Entries(
              [entries, header]()
              {
                  ...
                  return std::shared_ptr<const std::vector<std::shared_ptr<SfxScriptEntry>>>(
                      result);
              }())
    {
    }
    DgnFile::DgnFile(std::string name,
        DgnHeader header,
    ...
    DgnFileEntry::DgnFileEntry(std::uint16_t sfxId,
        ...
        : SfxId(static_cast<MphRead::SfxId>(sfxId)), Data1(std::move(data1)),
          Data2(std::move(data2)), Data3(std::move(data3)), Data4(std::move(data4))
    {
    }
        ...
        const std::optional<std::string>& prefix)
    {
        ...
        for (const std::shared_ptr<SoundSample>& sample : *samples)
        {
            ...
            {
                std::cout << "[" << sample->Id << "] WaveExportException: " << ex.what()
                          << std::endl;
            }
        }
    }
    void SoundRead::ExportSample(std::int32_t id, bool adpcmRoundingError)
    {
        ...
        {
            throw std::out_of_range("Index was out of range. Must be non-negative and less than "
        ...
        const std::shared_ptr<SoundSample>& sample, bool adpcmRoundingError)
    {
        ...
        return DecodeWaveDataVector(
            sample->CreateSpan(), sample->Format, GetSampleCount(*sample), adpcmRoundingError);
    }
    void SoundRead::GetWaveData(std::span<const std::uint8_t> data,
        WaveFormat format,
        std::uint32_t sampleCount,
        bool adpcmRoundingError,
        BinaryWriter& writer)
        ...
        const std::optional<std::string>& prefix)
    {
        ...
        ExportAudio(
            *waveData, GetSampleCount(*sample), sample->SampleRate, sample->Format, id, prefix);
    }
    void SoundRead::WriteWavHeader(BinaryWriter& writer,
        std::uint32_t sampleCount,
        std::uint16_t sampleRate,
        WaveFormat format)
    {
        ...
        BinaryWriterExtensions::Write4(
            writer, static_cast<std::uint32_t>(sampleRate) * (bitsPerSample / 8U));
    ...
    std::shared_ptr<const std::vector<std::shared_ptr<SoundSample>>>
    SoundRead::ReadWfsSoundSamples()
    {
        ...
        const std::string& filename)
    {
        const std::string path =
            MphRead::Paths::Combine(MphRead::Paths::FileSystem(), "data", "sound", filename);
        ...
        for (const std::uint32_t offset : *offsets)
        {
            ...
            {
                const SoundSampleHeader header =
                    MphRead::Read::DoOffset<SoundSampleHeader>(bytes, offset);
                const std::uint64_t start =
                    static_cast<std::uint64_t>(offset) + sizeof(SoundSampleHeader);
                const std::uint32_t size =
                    (static_cast<std::uint32_t>(header.LoopStart) + header.LoopLength) * 4U;
    ...
    std::shared_ptr<const std::vector<std::shared_ptr<SoundSelectEntry>>>
    SoundRead::ReadBgmSelectList()
    {
        return ReadSelectList("BGMSELECTLIST.DAT");
    }
    std::shared_ptr<const std::vector<std::shared_ptr<SoundSelectEntry>>>
    SoundRead::ReadSfxSelectList()
    {
        ...
        const std::string& filename)
    {
        ...
        const std::int64_t offset =
            static_cast<std::int64_t>(count) * sizeof(RawSoundSelectEntry) + 4;
    ...
    {
        const std::string path =
            MphRead::Paths::Combine(MphRead::Paths::FileSystem(), "data", "sound", "SND3DLIST.DAT");
        const std::vector<std::uint8_t> storage = FileReadAllBytes(path);
        const std::span<const std::uint8_t> bytes(storage);
        const std::uint32_t count = MphRead::Read::SpanReadUint(bytes, 0);
#ifndef NDEBUG
    ...
    {
        const std::string path =
            MphRead::Paths::Combine(MphRead::Paths::FileSystem(), "data", "sound", "SNDTBLS.DAT");
        const std::vector<std::uint8_t> storage = FileReadAllBytes(path);
        const std::span<const std::uint8_t> bytes(storage);
        const std::uint32_t count = MphRead::Read::SpanReadUint(bytes, 0);
#ifndef NDEBUG
        ...
        const auto categories =
            MphRead::Read::ReadStrings(bytes, offset, static_cast<std::int32_t>(maxCategory) + 1);
    ...
    std::shared_ptr<const std::vector<std::shared_ptr<SfxScriptFile>>>
    SoundRead::ReadSfxScriptFiles()
    {
        const std::string path = MphRead::Paths::Combine(
        ...
        for (std::size_t i = 0; i < headers->size(); ++i)
        {
            ...
            const auto entries =
                MphRead::Read::DoOffsets<RawSfxScriptEntry>(bytes, header.Offset + 4U, entryCount);
    ...
    {
        const std::string path =
            MphRead::Paths::Combine(MphRead::Paths::FileSystem(), "data", "sound", "DGNFILES.DAT");
        const std::vector<std::uint8_t> storage = FileReadAllBytes(path);
        const std::span<const std::uint8_t> bytes(storage);
        const std::uint32_t fileCount = MphRead::Read::SpanReadUint(bytes, 0);
#ifndef NDEBUG
        ...
        for (std::size_t i = 0; i < headers->size(); ++i)
        {
            ...
            const auto rawEntries =
                MphRead::Read::DoOffsets<DgnEntry>(bytes, header.Offset + 4U, entryCount);
            ...
            for (const DgnEntry entry : *rawEntries)
            {
                ...
                entries->push_back(
                    std::make_shared<DgnFileEntry>(entry.SfxId, data1, data2, data3, data4));
    ...
    std::shared_ptr<SoundData> SoundRead::ReadSdat()
    {
...
#endif
        const BlockHeader symbHeader =
            MphRead::Read::DoOffset<BlockHeader>(bytes, sdatHeader.SymbolBlockOffset);
#ifndef NDEBUG
        ...
        const auto getNames = [](std::span<const std::uint8_t> localBytes, std::uint32_t offset)
        {
            ...
            const auto entryOffsets =
                MphRead::Read::DoOffsets<std::uint32_t>(localBytes, offset + 4U, count);
        ...
        const std::span<const std::uint8_t> symbBytes =
            ManagedSliceFromUint(bytes, sdatHeader.SymbolBlockOffset);
        ...
        std::vector<std::pair<std::string, std::shared_ptr<const std::vector<std::string>>>>
            seqarcNames;
        ...
        const auto seqarcs =
            MphRead::Read::DoOffsets<SeqArcEntries>(symbBytes, arcOffset + 4U, seqarcCount);
        ...
        const BlockHeader infoHeader =
            MphRead::Read::DoOffset<BlockHeader>(bytes, sdatHeader.InfoBlockOffset);
#ifndef NDEBUG
...
#endif
        const auto getStructs = []<typename T>(
                                    std::span<const std::uint8_t> localBytes, std::uint32_t offset)
        {
            auto results = std::make_shared<std::vector<T>>();
            const std::uint32_t count = MphRead::Read::SpanReadUint(localBytes, offset);
            const auto structOffsets =
                MphRead::Read::DoOffsets<std::uint32_t>(localBytes, offset + 4U, count);
        ...
        const std::span<const std::uint8_t> infoBytes =
            ManagedSliceFromUint(bytes, sdatHeader.InfoBlockOffset);
        const auto seqInfo =
            getStructs.template operator()<SeqInfo>(infoBytes, infoHeader.SeqOffset);
        const auto seqarcInfo =
            getStructs.template operator()<std::uint32_t>(infoBytes, infoHeader.SeqarcOffset);
        const auto bankInfo =
            getStructs.template operator()<BankInfo>(infoBytes, infoHeader.BankOffset);
        const auto wavearcInfo =
            getStructs.template operator()<std::uint32_t>(infoBytes, infoHeader.WavearcOffset);
        const auto playerInfo =
            getStructs.template operator()<PlayerInfo>(infoBytes, infoHeader.PlayerOffset);
        const auto strmPlayerInfo =
            getStructs.template operator()<StrmPlayerInfo>(infoBytes, infoHeader.StrmPlayerOffset);
        const auto strmInfo =
            getStructs.template operator()<StrmInfo>(infoBytes, infoHeader.StrmOffset);
        (void)playerInfo;
        (void)strmPlayerInfo;
        ...
        for (std::int32_t i = 0; ManagedIntLessThanUInt(i, groupCount); i = ManagedAdd(i, 1))
        {
            ...
            groupOffset += 4U;
            groupInfo.push_back(
                MphRead::Read::DoOffsets<GroupItemInfo>(infoBytes, groupOffset, itemCount));
...
#endif
        fatOffset += 12U;
        const auto fatEntries =
            MphRead::Read::DoOffsets<SdatFatEntry>(bytes, fatOffset, fatHeader.Count);
        const SdatFileHeader fileHeader =
            MphRead::Read::DoOffset<SdatFileHeader>(bytes, sdatHeader.FileBlockOffset);
#ifndef NDEBUG
        ...
        { filesRead = std::bit_cast<std::int32_t>(static_cast<std::uint32_t>(filesRead) + 1U); };
        ...
        for (std::size_t i = 0; i < strmInfo->size(); ++i)
        {
            ...
            const SoundStreamHeader header =
                MphRead::Read::DoOffset<SoundStreamHeader>(bytes, entry.Offset);
#ifndef NDEBUG
...
#endif
            auto channels =
                std::make_shared<std::vector<std::shared_ptr<std::vector<std::uint8_t>>>>();
            std::uint32_t channelSize =
                header.BlockSamples * (header.BlockCount - 1U) + header.LastBlockSamples;
            if (format == WaveFormat::ADPCM)
            {
                channelSize *= 2U;
            }
            for (std::uint32_t j = 0; j < header.Channels; ++j)
            {
                    ...
                    k = ManagedAdd(k, 1))
                {
                    ...
                    if (static_cast<std::int64_t>(k) ==
                        static_cast<std::int64_t>(header.BlockCount - 1U))
                    {
                    ...
                    else
                    {
                        ...
                        if (j > 0 && static_cast<std::int64_t>(k) ==
                                         static_cast<std::int64_t>(header.BlockCount - 2U))
                        {
