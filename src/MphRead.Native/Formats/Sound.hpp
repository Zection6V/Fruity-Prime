...
namespace MphRead::Formats::Sound
{
    ...
    namespace FhSoundDependency
    {
        ...
        [[nodiscard]] std::shared_ptr<SoundSample> CreateFhSoundSample(std::uint32_t id,
            std::uint32_t offset,
    ...
    {
        template <typename T, std::size_t N, typename TWire = T> class ByValArray final
        {
        public:
            using value_type = T;
            ...
            explicit ByValArray(const std::array<TWire, N>& wire) : _wire(wire)
            {
                Materialize();
            }
            ByValArray(const ByValArray& other) : _wire(other.WireValues())
            {
            ...
            [[nodiscard]] std::shared_ptr<ManagedStorage> Managed() const
            {
                return TryGetStorage();
            }
    ...
    private:
        static void GetWaveData(std::span<const std::uint8_t> data,
            WaveFormat format,
        ...
        static void ExportSample(const std::shared_ptr<SoundSample>& sample,
            bool adpcmRoundingError = false,
            const std::optional<std::string>& prefix = std::nullopt);

    public:
        static void WriteWavHeader(BinaryWriter& writer,
            std::uint32_t sampleCount,
            std::uint16_t sampleRate,
            WaveFormat format);

    private:
        static void ExportAudio(std::span<const std::uint8_t> waveData,
            std::uint32_t sampleCount,
            std::uint16_t sampleRate,
            WaveFormat format,
    ...
    public:
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSample>>>
        ReadSoundSamples();
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSample>>>
        ReadWfsSoundSamples();
    ...
    private:
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSample>>>
        ReadSoundSamples(const std::string& filename);

    public:
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSelectEntry>>>
        ReadBgmSelectList();
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSelectEntry>>>
        ReadSfxSelectList();
    ...
    private:
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSelectEntry>>>
        ReadSelectList(const std::string& filename);
    ...
    public:
        ...
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<MusicTrack>>>
        ReadInterMusicInfo();
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SfxScriptFile>>>
        ReadSfxScriptFiles();
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<DgnFile>>>
        ReadDgnFiles();
    ...
    public:
        ...
        ReadFhGlobalSfx();
        [[nodiscard]] static std::shared_ptr<const std::vector<std::shared_ptr<SoundSample>>>
        ReadFhSoundFile(const std::string& filename);
    };
    struct SdatHeader
    {
        ...
        [[nodiscard]] static SdatHeader FromMarshaledBytes(
            const std::array<std::uint8_t, 64>& bytes);

    private:
        SdatHeader(SoundNativeRuntime::Char4 type,
            std::uint32_t magic,
    ...
    struct BlockHeader
    {
        ...
        [[nodiscard]] static BlockHeader FromMarshaledBytes(
            const std::array<std::uint8_t, 64>& bytes);

    private:
        BlockHeader(SoundNativeRuntime::Char4 type,
    ...
    struct SdatFatHeader
    {
        ...
        [[nodiscard]] static SdatFatHeader FromMarshaledBytes(
    ...
    struct SdatFileHeader
    {
        ...
        [[nodiscard]] static SdatFileHeader FromMarshaledBytes(
            const std::array<std::uint8_t, 16>& bytes);

    private:
        SdatFileHeader(SoundNativeRuntime::Char4 type,
            std::uint32_t size,
            std::uint32_t count,
    ...
    struct StrmPlayerInfo
    {
        ...
        [[nodiscard]] static StrmPlayerInfo FromMarshaledBytes(
            const std::array<std::uint8_t, 24>& bytes);

    private:
        StrmPlayerInfo(std::uint8_t channelCount,
    ...
    struct SoundStreamHeader
    {
        ...
        [[nodiscard]] static SoundStreamHeader FromMarshaledBytes(
            const std::array<std::uint8_t, 64>& bytes);

    private:
        SoundStreamHeader(SoundNativeRuntime::Char4 type,
            std::uint32_t magic,
    ...
    public:
        ...
        explicit SoundData(
            std::shared_ptr<const std::vector<std::shared_ptr<SoundStream>>> streams);
    ...
    public:
        ...
        DgnFileEntry(std::uint16_t sfxId,
    ...
    static_assert(
        std::is_standard_layout_v<RawSoundSelectEntry> && sizeof(RawSoundSelectEntry) == 16);
    ...
    static_assert(
        std::is_standard_layout_v<RawSoundTableEntry> && sizeof(RawSoundTableEntry) == 12);
