...
// Enums from Metadata/, Formats/ and Entities/ that the rest of the native
// tree did not already declare.  Transliterated from the managed sources so
// the ordinals cannot drift; the values are part of the cartridge's own
// tables and of the wire format.
...
// SoundMeta.cs
enum class VoiceId : std::int32_t {
    None = -1,
...
// SoundMeta.cs
enum class MusicId : std::int32_t {
    Invalid = -1,
    None = 0,
    SEQ_YELLOW_M1 = 1,
    SEQ_AMBIENT_1_M2 = 2,
...
// PlayerEntity.cs
enum class KandenAltAnim : std::uint8_t {
    Idle = 0,
    TailOut = 1,
    TailIn = 2
};
...
// PlayerEntity.cs
enum class NoxusAltAnim : std::uint8_t {
    Extend = 0
};

// PlayerEntity.cs
enum class SyluxAltAnim : std::uint8_t {
    Idle = 0
};

// PlayerEntity.cs
enum class SpireAltAnim : std::uint8_t {
    Attack = 0
};
