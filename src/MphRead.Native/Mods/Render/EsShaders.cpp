...
namespace
{
    // SHA256.HashData over the text's bytes, as lower-case hex. The digest
    // is reproduced once, in NativeRuntime/System/Cryptography.
    std::string Sha256Hex(const std::string& input)
    {
        const std::array<std::uint8_t, 32> digest = MphRead::NativeRuntime::Sha256HashData(
            std::span<const std::uint8_t>(
                reinterpret_cast<const std::uint8_t*>(input.data()), input.size()));
        std::string result = MphRead::NativeRuntime::ConvertToHexString(digest);
