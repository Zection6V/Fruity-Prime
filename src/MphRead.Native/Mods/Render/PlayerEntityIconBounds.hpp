...
struct IconBounds {
    ...
    IconBounds& operator=(const IconBounds& other) noexcept;
...
template <typename T>
concept ByteReadOnlyList = std::ranges::sized_range<T>
    && requires(const T& data, std::int32_t index) {
        requires std::integral<std::remove_cvref_t<decltype(std::ranges::size(data))>>;
        requires std::same_as<
            std::remove_cvref_t<decltype(data[index])>,
            std::uint8_t
        >;
    };
...
class PlayerEntity {
public:
    static IconBounds ModIconBounds(
...
private:
    using CountCallback = std::int32_t (*)(const void*);
    using ReadCallback = std::uint8_t (*)(const void*, std::int32_t);

    template <Detail::ByteReadOnlyList T>
    static std::int32_t CountAdapter(const void* data)
    {
        return static_cast<std::int32_t>(
            std::ranges::size(*static_cast<const T*>(data))
        );
    }
    ...
    static IconBounds ModIconBoundsCore(
        const void* data,
        CountCallback count,
        ReadCallback read,
        std::int32_t frame,
