...
namespace
{
    ...
    [[nodiscard]] float PreviewValue(
        const MphRead::Mods::MapGen::MapPreview* preview,
        bool position,
        std::size_t index)
    {
        if (preview == nullptr)
        ...
        const std::vector<float>* values
            = position ? preview->Position() : preview->Target();
        if (values == nullptr)
        {
            throw System::NullReferenceException();
        }
...
{
    bool ThumbnailCapture::_describedContext = false;

    RendererPlatform::WindowSettings ThumbnailCapture::GameSettings()
    {
    ...
    RendererPlatform::WindowSettings ThumbnailCapture::WindowSettings(
        std::int32_t width,
        std::int32_t height)
    {
        ...
        : _window(RendererPlatform::CreateWindow(WindowSettings(width, height)))
    {
        ...
        _scene->AddRoom(_roomKey, GameMode::Battle, 1);
    }

    ThumbnailCapture::~ThumbnailCapture() = default;

    MphRead::Scene& ThumbnailCapture::Scene() const
    {
        return *_scene;
    }

    bool ThumbnailCapture::Succeeded() const noexcept
    {
        return _captured;
    }
    ...
    void ThumbnailCapture::IsVisible(bool value)
    {
    ...
    void ThumbnailCapture::OnLoad()
    {
        ...
        if (!_describedContext)
        {
            ...
            line += Update::BuildVersion::Display();
            line += " / ";
            line += EntryAssemblyVersion();
            line += ", ";
            line += ScreenCapture::DescribeContext();
            line += ", window asked ";
            line += std::to_string(_asked.X);
            line += "x";
            line += std::to_string(_asked.Y);
            line += ", got ";
            line += std::to_string(ClientSize().X);
            line += "x";
    ...
    void ThumbnailCapture::ApplyPreviewCamera()
    {
        for (const std::shared_ptr<MapGen::MapDefinition>& definition
            : MapGen::CustomRooms::Definitions())
        {
            if (!definition)
            {
                throw System::NullReferenceException();
            ...
            const MapGen::MapPreview* preview = definition->Preview();
            if (preview != nullptr
                && CompareOrdinalIgnoreCase(definition->Name(), _roomKey) == 0)
            {
                const float positionX = PreviewValue(definition->Preview(), true, 0);
                const float positionY = PreviewValue(definition->Preview(), true, 1);
                const float positionZ = PreviewValue(definition->Preview(), true, 2);
                const OpenTK::Mathematics::Vector3 position(
                    positionX, positionY, positionZ);

                const float targetX = PreviewValue(definition->Preview(), false, 0);
                const float targetY = PreviewValue(definition->Preview(), false, 1);
                const float targetZ = PreviewValue(definition->Preview(), false, 2);
                const OpenTK::Mathematics::Vector3 target(
                    targetX, targetY, targetZ);

                _scene->SetPreviewCamera(position, target);
                return;
            }
        }
    }

    void ThumbnailCapture::OnRenderFrame(
        const RendererPlatform::FrameEventArgs& args)
    {
        ...
        MphRead::Scene* captureScene = _scene.get();
        const std::string path = ThumbnailGenerator::PathFor(_roomKey);
        _captured = ScreenCapture::Save(captureScene, path);

        if (_captured)
        {
            std::string line = _roomKey;
            line += ": captured at ";
            line += std::to_string(_scene->Size().X);
            line += "x";
            line += std::to_string(_scene->Size().Y);
            if (_attempts > 0)
            {
                line += " on attempt ";
        ...
        if (!_captured)
        {
            ...
            if (!IsVisible())
            {
                ...
                ThumbnailLog::Write(
                    _roomKey
                    + ": showing the window and retrying -- "
    ...
    void ThumbnailCapture::OnClosing()
    {
        _scene->DoCleanup();
    ...
    bool ThumbnailCapture::CaptureRoom(
        const std::string& roomKey,
        std::int32_t width,
        std::int32_t height)
    {
        try
        {
            ...
            ThumbnailCapture window(roomKey, width, height);
            window.Run();
            return window.Succeeded();
        }
        catch (const std::exception& exception)
        {
            std::cout << "[thumbnails] failed " << roomKey
                      << ": " << exception.what() << std::endl;
            ThumbnailLog::Write(
                roomKey + ": threw " + ExceptionTypeName(exception)
                + ": " + exception.what());
            return false;
        }
    }
