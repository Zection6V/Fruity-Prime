...
namespace {
constexpr std::string_view kVertexShader = R"MPH_SHADER(
#version 120
...
constexpr std::string_view kFragmentShader = R"MPH_SHADER(
#version 120
uniform bool use_texture;
uniform bool fog_enable;
uniform vec4 fog_color;
...
constexpr std::string_view kRttVertexShader = R"MPH_SHADER(
#version 120
...
constexpr std::string_view kRttFragmentShader = R"MPH_SHADER(
#version 120
...
constexpr std::string_view kCelFragmentShader = R"MPH_SHADER(
#version 120
...
constexpr std::string_view kShiftFragmentShader = R"MPH_SHADER(
#version 120
...
std::string_view vertex_shader() noexcept { return kVertexShader; }
std::string_view fragment_shader() noexcept { return kFragmentShader; }
std::string_view rtt_vertex_shader() noexcept { return kRttVertexShader; }
std::string_view rtt_fragment_shader() noexcept { return kRttFragmentShader; }
std::string_view cel_fragment_shader() noexcept { return kCelFragmentShader; }
std::string_view shift_fragment_shader() noexcept { return kShiftFragmentShader; }

std::string_view vertex_source(Program program) noexcept {
    switch (program) {
    case Program::RttVertex:
        return kRttVertexShader;
    case Program::Vertex:
    case Program::Fragment:
    case Program::RttFragment:
    case Program::CelFragment:
    case Program::ShiftFragment:
        return kVertexShader;
    }
    return kVertexShader;
}

std::string_view fragment_source(Program program) noexcept {
    switch (program) {
    case Program::Fragment:
        return kFragmentShader;
    case Program::RttFragment:
        return kRttFragmentShader;
    case Program::CelFragment:
        return kCelFragmentShader;
    case Program::ShiftFragment:
        return kShiftFragmentShader;
    case Program::Vertex:
    case Program::RttVertex:
        return {};
    }
...
bool has_required_entry_points(Program program) noexcept {
    const auto vertex = vertex_source(program);
    const auto fragment = fragment_source(program);
    return !vertex.empty() && !fragment.empty()
        && vertex.find("void main") != std::string_view::npos
        && fragment.find("void main") != std::string_view::npos;
}
