...
namespace MphRead::Text
{
    ...
    std::string Strings::ReplaceNonAscii(const std::string &value)
    {
        ...
        for (std::size_t i = 0; i < value.size(); i++)
        {
            ...
            else
            {
                ...
                const std::int32_t index = static_cast<std::int32_t>(next & 0x3FU) | (static_cast<std::int32_t>(c & 0x1FU) << 6);
    ...
    const std::shared_ptr<const std::vector<std::string>> StringTables::_all = std::make_shared<const std::vector<std::string>>(std::vector<std::string>{
